...
interface DelayNode extends EffectNode {
  delayTime: { value: string };
}
...
function buildSynthOptions(track: Track): object {
  ...
  if (track.instrument === "drum") {
    const presets: Record<string, object> = {
      ...
      Deep: {
        pitchDecay: 0.12,
        octaves: 10,
        envelope: { attack: 0.001, decay: 0.8, sustain: 0, release: 1.8 },
      },
      Click: {
        pitchDecay: 0.005,
        octaves: 3,
        envelope: { attack: 0.001, decay: 0.05, sustain: 0, release: 0.2 },
      },
      Crunch: {
        pitchDecay: 0.06,
        octaves: 5,
  ...
  if (track.instrument === "bass") {
    ...
    return {
      ...
      filter: {
        ...
        type: "lowpass",
        rolloff: -24,
        frequency: sound === "Growl" ? 800 : 1200,
      },
  ...
  if (track.instrument === "pad") {
    const presets: Record<string, object> = {
      ...
      Soft: {
        oscillator: { type: "sine" },
        envelope: { attack: 0.6, decay: 0.4, sustain: 0.5, release: 2.5 },
      },
    };
...
export class AudioEngine {
  ...
  setDelay(wet: number, time?: string): void {
    ...
    if (this.masterDelay) {
      ...
      this.masterDelay.delayTime.value = this.delayTime;
    }
  }
  ...
  private async ensureMasterBus(): Promise<void> {
    ...
    const Filter = tone.Filter as unknown as new (
      freq: number,
      type: string,
    ) => FilterNode;
    const Reverb = tone.Reverb as unknown as new (
      options: object,
    ) => EffectNode;
    const FeedbackDelay = tone.FeedbackDelay as unknown as new (
      delayTime: string,
      feedback: number,
    ) => DelayNode;
    const Analyser = tone.Analyser as unknown as new (
      type: string,
      size: number,
    ) => AnalyserNode;
    ...
    (this.masterFilter as unknown as ToneNode).connect(
      this.masterReverb as unknown as ToneNode,
    );
    (this.masterReverb as unknown as ToneNode).connect(
      this.masterDelay as unknown as ToneNode,
    );
    (this.masterDelay as unknown as ToneNode).connect(
  ...
  private reconcileVoices(project: StudioProject): void {
    ...
    project.tracks.forEach((track) => {
      ...
      (panner as unknown as ToneNode).connect(
        this.masterFilter as unknown as ToneNode,
      );
  ...
  private createPanner(volume: number): VolumeNode {
    if (!this.tone) throw new Error("Tone not loaded.");
    const Volume = this.tone.Volume as unknown as new (
      vol: number,
    ) => VolumeNode;
