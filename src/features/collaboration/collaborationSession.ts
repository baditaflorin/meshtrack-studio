import { cloneProject, safeParseProject, type StudioProject } from '../studio/project'
...
const SIGNALING_SERVERS = ['wss://y-webrtc-eu.fly.dev', 'wss://signaling.yjs.dev']
...
export class CollaborationSession {
  ...
  private async open(project: StudioProject): Promise<void> {
    ...
    const provider = new WebrtcProvider(this.roomName, doc, { signaling: SIGNALING_SERVERS }) as Provider
  ...
  private applySnapshot(rawSnapshot: unknown): void {
    ...
    if (!isSharedSnapshot(parsedSnapshot) || parsedSnapshot.clientId === this.clientId) {
      return
    }
...
function isSharedSnapshot(candidate: unknown): candidate is SharedSnapshot {
  ...
  return typeof snapshot.clientId === 'string' && safeParseProject(snapshot.project) !== null
}
...
function readPresence(candidate: unknown): PeerPresence | null {
  ...
  if (typeof state.peer?.clientId !== 'string' || typeof state.peer.name !== 'string') {
    return null
  }
