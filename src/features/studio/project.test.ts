...
describe('studio project model', () => {
  it('creates a valid default project with four tracks', () => {
    ...
    expect(project.tracks.every((track) => track.pattern.length === STEP_COUNT)).toBe(true)
  })
  ...
  it('clears and randomizes pattern data', () => {
    ...
    expect(cleared.tracks.flatMap((track) => track.pattern).every((step) => !step)).toBe(true)
    expect(randomized.tracks.every((track) => track.pattern.length === STEP_COUNT)).toBe(true)
  })
})
