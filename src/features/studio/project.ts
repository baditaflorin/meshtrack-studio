...
export const instrumentOptions = ['lead', 'bass', 'pad', 'pluck', 'drum'] as const
...
const seedTracks = [
  ...
  },
  {
    ...
    note: 'C2',
    volume: -10,
    pattern: [true, false, false, true, false, false, true, false, true, false, false, true, false, true, false, false],
  },
  {
    name: 'Glass lead',
    color: '#0984e3',
    ...
    note: 'G4',
    volume: -13,
    pattern: [false, false, true, false, false, true, false, false, false, false, true, false, true, false, false, true],
  },
  {
    name: 'Warm pad',
    color: '#fdcb6e',
...
export function toggleStep(project: StudioProject, trackId: string, stepIndex: number): StudioProject {
  return updateTrack(project, trackId, (track) => ({
    ...track,
    pattern: track.pattern.map((step, index) => (index === stepIndex ? !step : step)),
  }))
}

export function setTrackVolume(project: StudioProject, trackId: string, volume: number): StudioProject {
  return updateTrack(project, trackId, (track) => ({
    ...track,
...
export function setTrackMuted(project: StudioProject, trackId: string, muted: boolean): StudioProject {
  return updateTrack(project, trackId, (track) => ({
    ...track,
    muted,
  }))
...
export function setTrackSolo(project: StudioProject, trackId: string, solo: boolean): StudioProject {
  return updateTrack(project, trackId, (track) => ({
    ...track,
    solo,
  }))
...
export function setProjectBpm(project: StudioProject, bpm: number): StudioProject {
  return touchProject({
    ...project,
...
export function setProjectTitle(project: StudioProject, title: string): StudioProject {
  ...
  return touchProject({
    ...
    tracks: project.tracks.map((track, trackIndex) => ({
      ...
      pattern: track.pattern.map((_, stepIndex) => {
        ...
        return Math.random() < density || (downbeat && trackIndex < 2 && Math.random() < 0.55)
      }),
    })),
  })
}
